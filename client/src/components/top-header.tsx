  ...
  return (
    <header className="flex items-center justify-between gap-4 px-7 py-5 border-b border-white/[0.04]">
      ...
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-[11px] text-ink-mute">monitoring</span>
      </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-2.5 pl-1 pr-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.06]">
          <div className="relative h-7 w-7 rounded-full bg-gradient-to-br from-accent-lavender to-accent-pink flex items-center justify-center text-[11px] font-bold text-canvas">
            {username[0]?.toUpperCase() ?? "?"}
            <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-level-ok ring-2 ring-canvas pulse-lavender" />
          </div>
          ...
          <button onClick={onLogout} className="ml-1 text-ink-mute hover:text-accent-pink transition" title="logout">
            <LogOut size={12} />
          </button>
        </div>
      </div>
